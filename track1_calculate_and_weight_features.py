...
a439 = early_test.loc[:9, 'ProblemID'].values
a492 = early_test.loc[20:29, 'ProblemID'].values

# =============================================================================
# ## avg lor
# np.mean(np.abs(lor_df.loc[[x in a439 for x in lor_df.ProblemID],:].iloc[:,1:].values))
# np.mean(np.abs(lor_df.loc[[x in a492 for x in lor_df.ProblemID],:].iloc[:,1:].values))
# ## avg weights
# temp = lor_df.copy()
# temp.iloc[:,1:] = temp.iloc[:,1:]/temp.iloc[:,1:].sum()*30
# np.mean(np.abs(temp.loc[[x in a439 for x in temp.ProblemID],:].iloc[:,1:].values))
# np.mean(np.abs(temp.loc[[x in a492 for x in temp.ProblemID],:].iloc[:,1:].values))
# =============================================================================
...
def weight_feature(weight_matrix, early, raw_data_early, use_order_weight = False, weight_vector = False, onlyS19=False): 
    ...
    # if all weights in weight_matrix are the same, it means problem similiarity weight
...
combine_order_weight = False
if combine_order_weight:
    comb = "order_"
else:
    comb = ""    
...
for f in weight_files:
    ...
    weight_matrix = pd.read_csv(weight_path+f)
    weight_matrix[weight_matrix < 0] = np.nan
    weight_matrix.fillna(value = weight_matrix.min(), inplace = True)
    
    weight_matrix.columns = ['ProblemID']+later_problems
    X_train, X_test = extract_weight_features(
                         ...
                         raw_data_early, 
                         late_problem_df, 
                         use_order_weight = combine_order_weight,
                         weight_vector = True,
                         onlyS19 = onlyS19)
    ...
    X_train.to_csv(outpath+'X_train_'+comb+f, index=False)
    X_test.to_csv(outpath+'X_test_'+comb+f, index=False)
#### weight features by  difficulty
weight_matrix = lor_df.copy()
...
X_train.to_csv(outpath+'X_train_'+comb+'difficulty.csv', index=False)
X_test.to_csv(outpath+'X_test_'+comb+'difficulty.csv', index=False)


#### weight features by order
# only use problem order weight, so all early problems 
# should have the same weight pertain to later problems
weight_matrix.iloc[:,1:] = 1
...
X_train.to_csv(outpath+'X_train_order.csv', index=False)
X_test.to_csv(outpath+'X_test_order.csv', index=False)

#### combine order, difficulty, prompt, and code weights
#for similarity in ['cosine', 'Euclidean', 'pearson']:
similarity = 'cosine'
weight_matrix = [lor_df.copy()]
temp_file = [x for x in weight_files if similarity in x ]
for f in temp_file:
    temp_weight = pd.read_csv(weight_path+f)
...
X_train.to_csv(outpath+'X_train_'+'order_'+similarity+'_difficulty.csv', index=False)
X_test.to_csv(outpath+'X_test_'+'order_'+similarity+'_difficulty.csv', index=False)
...
X_train.to_csv(outpath+'X_train_no_weight.csv', index=False)
X_test.to_csv(outpath+'X_test_no_weight.csv', index=False)

pd.Series(y_train, name='label').to_csv(outpath+'y_train_all.csv', index=False)
